...
class process_analysis_tool(BaseTool):
    ...
    def _run(self, chosen_approach:str, abstraction: str) -> str:
        # To Improve!
        ...
        formatted_prompt = process_analysis_prompt.format(user_prompt="question", model=chosen_approach,
                                                          characteristics_model=model_analysis,
                                                          abstraction=abstraction)
        response = model.invoke(formatted_prompt)
        print("the conclusion is: ", response.content)
