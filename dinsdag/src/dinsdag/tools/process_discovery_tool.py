...
@tool
def process_discovery_tool(focus:str, filename:str, process:str) -> str:
    """Takes the abstraction of the event log"""
    ...
    if approach not in ["DFG", "Variants", "Temporal Profile"]:
        print("Unable to find the right approach, try again with another analysis or explain in more detail what needs to be analysed")
        print("Quit the program manually please. ")
    print(approach, "is the approach")
    workingDirectory = os.path.dirname(sys.argv[0])
    path = os.path.join(workingDirectory, "tools","Event_logs", filename)
    file_read = Event_log_readings(path, filename)

    if approach == "DFG":
        print("goint to take an abstraction")
        abstraction = pm4py.llm.abstract_dfg(file_read)
        print("file is abstracted")
    ...
    print("model analysis taken")
    filled_template = process_analysis_prompt.format(user_prompt=focus, model=approach, process=process,focus=focus,
                                                         characteristics_model=model_analysis)
    prompt = filled_template + abstraction
    print('going to ask the prompttt')
    resp = pm4py.llm.openai_query(prompt, api_key=os.getenv('OPENAI_API_KEY'), openai_model="gpt-4o")
    print("The process is analysed.")
    return resp