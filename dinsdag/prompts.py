choosing_approach_prompt ="""
    ...
    The options are (1) "DFG", (2) "Temporal Profile" and (3) "Variants".
     ...
     Base your answer on the the user's input:
    {user_query}. 
     (if no relevant information is available in the user input, please use the "DFG" approach):
...
Solver_prompt = """Use the process analysis provided and analyse use the top 3 steps identified. 
Look through the internet to answer the question provided. 

Make sure that you include a reference to a step every time you answer the question!
The steps are: {process_analysis}.
...
The lists of process inefficiecies and reports: {report}
...
#FINISH THIS PROMPT
#make it an f-string?
DK_company_prompt = """You are a company expert. Search the 
internet for useful facts and suggestions that link the P&G (proctor and gamble) in question to bottlenecks in production
"""

#FINISH THIS PROMPT
DK_process_prompt  = """You are a company expert. Use the process mining information available to search the 
internet for useful facts and suggestions that link the company in question to ...
"""